...
namespace Sledge.BspEditor.Tools.Texture
{
    public partial class TextureBrowser : Form, IManualTranslate
    {
        ...
        public TextureBrowser(MapDocument document)
        {
            ...
            Triggershow.Checked = GetMemory("Triggershow", false);
        ...
        public void Translate(ITranslationStringProvider strings)
        {
            ...
            this.InvokeLater(() =>
            {
                ...
                Triggershow.Text = strings.GetString(prefix, "TriggerShow");
        ...
        private void TriggerShowChanged(object sender, EventArgs e)
        {
            SetMemory("TriggerShow", Triggershow.Checked);
            UpdateTextureList();
        }
        ...
        {
            var list = FavouritesTree.SelectedNode != null
                ? GetFavouriteFolderTextures()
                : GetPackageTextures();
            ...
            {
                var textureNames = new HashSet<string>(_document.Map.Root.FindAll()
                    .SelectMany(x => x.Data.OfType<ITextured>())
                    .Select(x => x.Texture.Name).Distinct());
                list = list.Where(x => textureNames.Contains(x, StringComparer.InvariantCultureIgnoreCase));
            }

            list = list.Where(x => !x.Equals("signature", StringComparison.OrdinalIgnoreCase));

            if (!GetMemory("TriggerShow", true))
            {
                var excludedTriggerTextures = new[] { "hint", "skip", "null", "bevel", "aaatrigger", "sky", "origin", "skybox", "solidhint", "monitor", "clip" };
