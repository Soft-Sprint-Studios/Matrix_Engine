...
namespace Sledge.BspEditor.Primitives.MapObjects
{
	public static class MapObjectExtensions
	{
		...
		public static Vector3? GetIntersectionPoint(this IMapObject obj, Line line, IgnoreOptions options, bool ignoreDirection = false)
		{
			if (obj is Solid solid)
			{
				return solid.Faces.Where(x => (!(options.HasFlag(IgnoreOptions.IgnoreClip) && x.Texture.Name.ToLower().StartsWith("clip"))
				&& !(options.HasFlag(IgnoreOptions.IgnoreNull) && x.Texture.Name.ToLower().StartsWith("null"))))
