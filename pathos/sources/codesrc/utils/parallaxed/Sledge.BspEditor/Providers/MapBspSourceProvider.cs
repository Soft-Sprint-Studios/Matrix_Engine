...
namespace Sledge.BspEditor.Providers
{
	...
	public class MapBspSourceProvider : IBspSourceProvider
	{
		...
		private static readonly string[] ExcludedKeys = new[] { "spawnflags", "classname", "origin", "wad", "mapversion", "editorversion" };
        ...
        private void WriteWorld(StreamWriter sw, Root world)
        {
            ...
            if (!ed.Properties.ContainsKey("editorversion"))
            {
                DateTime startDate = new DateTime(2024, 1, 1);
                DateTime today = DateTime.Today;
                int daysFromStart = (today - startDate).Days;
                WriteProperty(sw, "editorversion", daysFromStart.ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                WriteProperty(sw, "editorversion", ed.Properties["editorversion"]);
            }

            foreach (var prop in ed.Properties)
            {
                if (prop.Key == "classname" || prop.Key == "spawnflags" || prop.Key == "mapversion" || prop.Key == "editorversion")
                    continue;
